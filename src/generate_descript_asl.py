...
def create_feature(args) -> None:
    ...
    if extract_hidden_states:
        ...
        suffix = "SLdescript"
    save_file = save_path / f"{args.dataset}_{suffix}_{split}.pt"
    ...
    if args.dry_run:
        print("Dry run: dataset paths are valid. Not loading the MLLM.")
        for vid_name, frame_idx, frame_path in all_entries[:10]:
            print(f"  {vid_name} frame {frame_idx}: {frame_path}")
        return
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        choices=["phoenix", "asl"],
        default="phoenix",
        help="Dataset to use for default frame path and output filename prefix.",
    )
    parser.add_argument(
        "--img_path",
        type=str,
        default=None,
        help=(
            "Root frame directory containing split folders, e.g. "
            "../Dataset/frames/ where ../Dataset/frames/train/<video_id>/*.jpg exists. "
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Only collect and print frame paths; do not load the MLLM or save features.",
    )
    parser.add_argument(
    ...
    default_img_paths = {
        "phoenix": "datasets/PHOENIX-2014-T-release-v3/PHOENIX-2014-T/features/fullFrame-210x260px/",
        "asl": "../Dataset/frames/",
    }

    if args.img_path is None:
        args.img_path = default_img_paths[args.dataset]

    args.img_path = str(pathlib.Path(args.img_path).expanduser())
