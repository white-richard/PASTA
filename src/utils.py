...
def count_parameters_in_MB(model):
    ...
    return sum(np.prod(v.size()) for name, v in model.named_parameters() if v.requires_grad) / 1e6
    ...
    for _ii, gloss in enumerate(raw_gloss):
        ...
        if noise_type == "omit":
            ...
            if random.uniform(0, 1) <= 1.0 and is_train:
                index = np.arange(
                    0,
                    len(text) - int(np.ceil(len(text) * (np.random.uniform(0, noise_rate, (1,))))),
                    1,
                    dtype=int,
                )
                noise_gloss = []
...
def data_augmentation(resize=(320, 240), crop_size=224, is_train=True):
    if is_train:
        left, top = (
            np.random.randint(0, resize[0] - crop_size),
            np.random.randint(0, resize[1] - crop_size),
        )
    else:
...
def visualization(atten_maps) -> None:
    ...
    for ii, att in enumerate(atten_maps):
        ...
        idx = [max(1, int(T**0.5)), T // max(1, int(T**0.5)), 1]

        for b in range(B):
            fig = plt.figure(figsize=(6 * idx[1], 6 * idx[0]))
            for t in range(T):
                ax = fig.add_subplot(*idx)
                cmp = att[ii, b, t].detach().cpu().numpy()
                sns.heatmap(
                    cmp,
                    cbar=(t == T - 1),
                    annot=False,
                ...
                idx[-1] += 1
            fig.savefig(os.path.join("./demo", f"Att_score_{ii}_batch_{b}.jpg"), dpi=fig.dpi)
            plt.close()
...
def param_groups_weight_decay(model: torch.nn.Module, weight_decay=1e-5, no_weight_decay_list=()):
